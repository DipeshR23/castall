  ...
  return (
    <div className="flex flex-col">
      ...
      <section className="relative flex flex-col items-center text-center pt-10 sm:pt-14 md:pt-20 lg:pt-24 pb-12 sm:pb-16 md:pb-20 lg:pb-24 px-4">
        ...
        {/* Main Heading */}
        <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-slate-900 dark:text-white mb-1.5 sm:mb-2 tracking-tight px-2">
        ...
        {/* Subtitle */}
        <p className="text-slate-500 dark:text-slate-400 max-w-md sm:max-w-lg mb-8 sm:mb-10 text-xs sm:text-sm md:text-base leading-relaxed px-4">
        ...
        <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 w-full max-w-lg sm:max-w-2xl px-4 sm:px-0">
          ...
          <button
            ...
            className="flex-1 rounded-2xl sm:rounded-3xl bg-white dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 px-8 py-4 sm:px-10 sm:py-5 md:px-12 md:py-6 text-base sm:text-lg md:text-xl font-semibold text-slate-700 dark:text-slate-200 shadow-sm hover:bg-slate-50 dark:hover:bg-slate-700 transition-all duration-150 hover:scale-[1.02] active:scale-[0.98] focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2"
          >
            <div className="flex items-center justify-center gap-2 sm:gap-3">
        ...
        <div className="flex flex-col sm:flex-row gap-6 sm:gap-8 w-full max-w-lg sm:max-w-2xl px-4 sm:px-0">
          <p className="flex-1 text-center text-xs sm:text-sm text-slate-400 dark:text-slate-500">Create a room and get a QR code</p>
          <p className="flex-1 text-center text-xs sm:text-sm text-slate-400 dark:text-slate-500">Join a room and share your screen</p>
        </div>
      </section>
      ...
      <section className="py-12 sm:py-16 md:py-24 lg:py-32 px-4">
        <div className="text-center mb-10 sm:mb-12 md:mb-16">
          <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-slate-900 dark:text-white mb-2 sm:mb-3">
            How it works
          </h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 sm:gap-8 md:gap-12 lg:gap-16">
            {steps.map((step) => (
              <div key={step.num} className="flex flex-col items-center text-center">
                ...
                {/* Icon */}
                <div className="w-14 h-14 sm:w-16 sm:h-16 md:w-20 md:h-20 rounded-xl sm:rounded-2xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm flex items-center justify-center mb-3 sm:mb-4">
                  <step.Icon className="h-7 w-7 sm:h-8 sm:w-8 md:h-10 md:w-10 text-slate-700 dark:text-slate-300" />
                </div>

                {/* Title */}
                <h3 className="text-sm sm:text-base md:text-lg font-semibold text-slate-900 dark:text-white leading-snug px-2">
                  {step.title}
                </h3>
              </div>
            ))}
      ...
      <section className="py-12 sm:py-16 md:py-24 lg:py-32 px-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 max-w-5xl mx-auto">
          {[
            ...
            <div
              key={feature.title}
              className="bg-white dark:bg-slate-800 rounded-xl sm:rounded-2xl p-5 sm:p-6 shadow-sm border border-slate-100 dark:border-slate-700 hover:shadow-md transition-all duration-150"
            >
