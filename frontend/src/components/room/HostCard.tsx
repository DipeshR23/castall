...
export default function HostCard({ roomCode, status, incomingDevice, onAccept, onReject, onReset }: HostCardProps) {
  ...
  return (
    <div className="w-full">
      <div className="w-full rounded-3xl sm:rounded-[32px] bg-white p-8 sm:p-10 md:p-12 shadow-sm border border-slate-100">
        <div className="flex flex-col items-center">
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-semibold text-slate-900 mb-2">Room Created 🎉</h2>
          <p className="text-base sm:text-lg text-slate-500 mb-6 sm:mb-8">Share this QR code to invite others</p>
          ...
          <div className="flex items-center justify-center gap-3 mb-4">
            <span className="text-xl sm:text-2xl font-bold text-primary tracking-wider">{roomCode}</span>
            <button
              type="button"
            ...
            >
              {copied ? <Copy className="h-4 w-4 sm:h-5 sm:w-5 text-success" /> : <Copy className="h-4 w-4 sm:h-5 sm:w-5 text-slate-400" />}
            </button>
          </div>
          
          <p className="text-sm sm:text-base text-slate-400 mb-6 sm:mb-8" role="status" aria-live="polite">
            Waiting for presenter to connect...
          </p>
          
          <div className="w-full space-y-3 sm:space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between py-3 sm:py-4 border-t border-slate-100 gap-2 sm:gap-4">
              <span className="text-sm sm:text-base text-slate-500">Share Link</span>
              <div className="flex items-center gap-2 sm:gap-3">
                <span className="text-sm sm:text-base text-slate-600 truncate max-w-[200px] sm:max-w-[300px]">{window.location.origin}/share?room={roomCode}</span>
                <button
                  type="button"
                  onClick={() => handleCopy(`${window.location.origin}/share?room={roomCode}`)}
                  className="rounded-lg p-2 hover:bg-slate-100 transition-colors min-h-[44px] min-w-[44px] flex items-center justify-center flex-shrink-0"
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between py-3 sm:py-4 border-t border-slate-100 gap-2 sm:gap-4">
              <span className="text-sm sm:text-base text-slate-500">Connected Devices</span>
      ...
      <div className="mt-6 sm:mt-8 flex justify-center">
        <button
          type="button"
          onClick={onReset}
          className="rounded-xl sm:rounded-2xl border-2 border-error text-error px-6 sm:px-8 py-3 sm:py-4 text-base sm:text-lg font-semibold hover:bg-error/5 transition-all duration-150 active:scale-[0.98] focus:outline-none focus:ring-2 focus:ring-error focus:ring-offset-2 min-h-[52px]"
        >
          End Room
        </button>
      </div>
